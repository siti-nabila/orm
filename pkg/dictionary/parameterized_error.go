...
var (
	...
	errorPack errorpackage.DictionaryPack
...
func init() {
	...
	errorPack = errorpackage.NewErrYamlPackage()
	if err := errorPack.LoadBytes(errorList); err != nil {
		panic(err)
	}
...
func UnsupportedTypeError(dialectName string) error {
	...
	return errorPack.Newf("err_placeholder_not_supported", dialectName)
}
...
func ErrNegativeValueUintError(colName string, value int64) error {
	...
	return errorPack.Newf("err_negative_value_uint", colName, value)
}
...
func ErrAdvInsConflictUnsupportedAction(actionName string) error {
	...
	return errorPack.Newf("err_adv_ins_conflict_unsupported_action", actionName)
}

func ErrScanTypeMismatch(colName string, value any) error {
	// return errScanTypeMismatch.(faults.Error).Render(colName, value)
	return errorPack.Newf("err_scan_type_mismatch", colName, value)
}
...
func ErrScanTypeIntMismatch(colName string, value any) error {
	...
	return errorPack.Newf("err_scan_type_int_mismatch", colName, value)
}
...
func ErrScanIntoColCountMismatch(expected, actual int) error {
	...
	return errorPack.Newf("err_scan_into_column_count_mismatch", expected, actual)
}
